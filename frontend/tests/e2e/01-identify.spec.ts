...
test("UC-05: пользователь скачивает PDF-отчёт идентификации", async ({ page }) => {
  ...
  // Если бэк не вернул результат за 30 секунд — пропускаем (на фазе 1 модели
  // в /models могут быть пустыми и пайплайн падает 500).
  try {
    await page.waitForURL(/\/identify\/results\//, { timeout: 30_000 });
  } catch {
  ...
  const exportButton = page.getByRole("button", { name: /Сохранить отчёт/i });
